...
def separate_punctuation(line: str):
    ...
    line = line.replace(" '", f" {single_opening_quote} ").replace(
        "' ", f" {single_closing_quote} "
    )
...
def process_lines(lines: list[str]):
    ...
    for line in lines:
        if len(line) > 3:  # Otherwise some lines are empty or just a period, etc
            i += 1
            # Lowercase the first alphabetic character, even if the sentence starts
            # with punctuation and/or whitespace (e.g. "- Usted ...").
            chars = list(line)
